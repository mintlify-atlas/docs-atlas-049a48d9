---
title: "Run the SuperCompress Local Development Server"
sidebarTitle: "Local Server"
description: "Spin up a local FastAPI server to serve the browser demo, playground, /v1/compress, and /dashboard without a Vercel deployment."
---
...
The response includes `compressed_text`, `original_tokens`, `kept_tokens`, `kv_savings_pct`, `line_annotations`, and — when `compare: true` — a `compare` map keyed by policy name.
