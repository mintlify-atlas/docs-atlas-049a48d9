---
title: "How SuperCompress Compresses LLM Context"
...
<Steps>
  <Step title="Tokenize + Feature Extraction (CPU)">
    The raw context string is split into lines, then each line is tokenized with a simple whitespace-and-punctuation splitter. For every token, `build_inference_records` in `local.py` builds a `TokenRecord` containing a **9-dimensional feature vector**:
    ...
    | 2 | `recency` — normalized position (`1 - age / seq_len`) |
    | 3 | `question_entity_match` — 1.0 if token appears in extracted question entities |
    | 4–7 | Semantic type one-hot: `CODE`, `COMMENT`, `CHAT`, `BOILERPLATE` |
